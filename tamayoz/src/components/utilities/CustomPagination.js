...
export default function CustomPagination({ pagination, onPageChange }) {
    ...
    return (
      ...
      >
        <div 
          style={{ transform: language === "en" ? "rotateY(180deg)" : "rotateY(0deg)" }} 
        ...
        {language === "ar" ? 
          <span>{pagination.pageCount} - {pagination.page + 1}</span> 
          : 
          <span>{pagination.page + 1} - {pagination.pageCount}</span>
        }
        <div 
          style={{ transform: language === "en" ? "rotateY(0deg)" : "rotateY(180deg)" }} 
          className={styles.arrowContainer}
          onClick={() => {
            console.log("clicked")
